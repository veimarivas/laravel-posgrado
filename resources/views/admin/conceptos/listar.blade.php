...
@section('admin')
    <style>
        ...
        .conceptos-header {
            ...
            margin-bottom: 28px;
            padding: 24px 28px;
            background: linear-gradient(135deg, var(--conc-primary) 0%, var(--conc-primary-dark) 100%);
        ...
        .conceptos-table tbody tr:last-child td {
            border-bottom: none;
        }
    ...
    <div class="container-fluid conceptos-page">
        <!-- Page Header -->
        <div class="conceptos-header">
            <div>
                <h1><i class="ri-file-list-3-line me-2"></i>Gestión de Conceptos de Pago</h1>
                <p>Administra los conceptos de pago disponibles</p>
            </div>
            @if (Auth::guard('web')->user()->can('conceptos.pagos.registrar'))
                <button type="button" class="btn-new-concepto" data-bs-toggle="modal" data-bs-target="#registrar">
        ...
        <div class="search-bar">
            <div class="search-wrapper">
                ...
                <input type="text" id="searchInput" class="form-control" placeholder="Buscar concepto de pago..." value="{{ request('search') ?? '' }}">
            </div>
        ...
        <div class="table-card">
            <div class="table-card-header">
                <h5><i class="ri-list-check me-2 text-muted"></i>Listado de Conceptos de Pago</h5>
            </div>
            <div class="table-responsive">
                <table class="conceptos-table">
                    ...
                    <tbody id="conceptosTableBody">
                        @include('admin.conceptos.partials.table-body')
                    </tbody>
                </table>
            </div>
            @if ($conceptos->total() > 0)
                <div class="table-footer">
                    <div class="results-count">
                        Mostrando <span class="fw-medium">{{ $conceptos->firstItem() }}</span> a
                        <span class="fw-medium">{{ $conceptos->lastItem() }}</span> de
                        <span class="fw-medium">{{ $conceptos->total() }}</span> resultados
                    </div>
                    <div class="pagination-container">
                        {{ $conceptos->appends(request()->input())->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            @endif
        </div>
    ...
    <div class="modal fade" id="registrar" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="background: var(--conc-primary-light);">
                    <h5 class="modal-title" style="color: var(--conc-primary); font-weight: 600;">
                        <i class="ri-add-line me-2"></i>Registrar Nuevo Concepto
                    </h5>
                ...
                <form id="addForm" class="needs-validation" novalidate>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nombre_registro" class="form-label">Nombre del Concepto <span class="text-danger">*</span></label>
                            <input type="text" id="nombre_registro" name="nombre" class="form-control" placeholder="Ej: Matrícula, Mensualidad, Materiales" required>
                            <div class="invalid-feedback">Por favor ingresa el nombre del concepto</div>
                            <small id="feedback_registro" class="form-text mt-1"></small>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn addBtn" style="background: var(--conc-primary); color: white;" disabled>
                            <i class="ri-save-3-line me-1"></i> Registrar Concepto
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="modalModificar" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="background: var(--conc-accent-light);">
                    <h5 class="modal-title" style="color: #b45309; font-weight: 600;">
                        <i class="ri-edit-line me-2"></i>Editar Concepto de Pago
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="updateForm" class="needs-validation" novalidate>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nombre_edicion" class="form-label">Nombre del Concepto <span class="text-danger">*</span></label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn updateBtn" style="background: var(--conc-accent); color: white;" disabled>
                            <i class="ri-refresh-line me-1"></i> Actualizar Concepto
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="modal-content">
                ...
                <form id="deleteForm">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <h5 style="font-weight: 600;">¿Estás seguro de eliminar este concepto?</h5>
                        <p class="text-muted mb-0">Esta acción no se puede deshacer.</p>
                    </div>
                    <div class="modal-footer justify-content-center">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btnDelete" style="background: var(--conc-danger); color: white;">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#nombre_registro').on('input', function() {
                ...
                debounceTimer = setTimeout(() => {
                    $.ajax({
                        ...
                        beforeSend: function() {
                            feedback.html('<i class="ri-loader-4-line spin me-1"></i> Verificando...');
                        },
                        success: function(res) {
                            if (res.exists) {
                                feedback.addClass('text-danger').html('<i class="ri-error-warning-line me-1"></i> Este concepto ya existe');
                                submitBtn.prop('disabled', true);
                                input.addClass('is-invalid').removeClass('is-valid');
                            } else {
                                feedback.addClass('text-success').html('<i class="ri-checkbox-circle-line me-1"></i> Nombre disponible');
                                submitBtn.prop('disabled', false);
                                input.addClass('is-valid').removeClass('is-invalid');
                            }
                        },
                        error: function() {
                            feedback.addClass('text-danger').html('<i class="ri-alert-line me-1"></i> Error al verificar el nombre');
                            submitBtn.prop('disabled', true);
                            input.addClass('is-invalid').removeClass('is-valid');
                        }
                    });
            ...
            $('#nombre_edicion').on('input', function() {
                ...
                debounceTimer = setTimeout(() => {
                    $.ajax({
                        url: "{{ route('admin.conceptos.verificaredicion') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            nombre: nombre,
            ...
            $('#addForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.conceptos.registrar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            showToast('error', res.msg || 'Error al registrar el concepto');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al registrar el concepto. Intenta nuevamente.';
            ...
            $('#updateForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.conceptos.modificar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            if (modal) { modal.hide(); } else { $('#modalModificar').modal('hide'); }
                            showToast('success', res.msg);
                            setTimeout(() => {
                                loadResults($('#searchInput').val().trim());
                            }, 300);
                        } else {
                            showToast('error', res.msg || 'Error al actualizar el concepto');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al actualizar el concepto. Intenta nuevamente.';
                        if (xhr.responseJSON?.errors?.nombre) {
                            errorMsg = xhr.responseJSON.errors.nombre[0];
                        } else if (xhr.responseJSON?.msg) {
                            errorMsg = xhr.responseJSON.msg;
            ...
            $('#deleteForm').submit(function(e) {
                ...
                const originalHtml = submitBtn.html();
                submitBtn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>Eliminando...');
                $.ajax({
                    url: "{{ route('admin.conceptos.eliminar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            var modal = bootstrap.Modal.getInstance(modalEl);
                            if (modal) { modal.hide(); } else { $('#modalEliminar').modal('hide'); }
                            showToast('success', res.msg);
                            setTimeout(() => {
                                loadResults($('#searchInput').val().trim());
                            }, 300);
                        } else {
                            showToast('error', res.msg || 'Error al eliminar el concepto');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al eliminar el concepto. Intenta nuevamente.';
            ...
            function loadResults(search = '') {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.html) {
                            $('#conceptosTableBody').html(response.html);
                            ...
                            if (response.total !== undefined) {
                                ...
                                $('#totalConceptosCounter').text(response.total);
                            }
                            $('[data-bs-toggle="tooltip"]').each(function() {
            ...
            $('#searchInput').on('input', function() {
                ...
                clearTimeout(debounceTimer);
                debounceTimer = setTimeout(() => { loadResults(searchTerm); }, 500);
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.html) {
                            ...
                            const newUrl = url + (search ? '&search=' + encodeURIComponent(search) : '');
                            window.history.pushState({}, '', newUrl);
                        }
                    },
                    error: function() { showToast('error', 'Error al cargar la página'); }
                });
            });
