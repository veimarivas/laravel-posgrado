        ...
        <div class="row mb-3">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between bg-light rounded-3 p-3">
                    <div>
                        <h4 class="mb-1 fw-bold text-dark">Gestión de Profesiones</h4>
                        <p class="text-muted mb-0">Administra las profesiones disponibles</p>
                    </div>

                    @if (Auth::guard('web')->user()->can('profesiones.registrar'))
                        <div class="page-title-right">
                            <button type="button" class="btn btn-primary btn-lg waves-effect waves-light"
                                ...
                                <i class="ri-add-line align-bottom me-1"></i> Nueva Profesión
                            </button>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-3">
                                <p class="text-muted mb-1">Total Profesiones</p>
                                <h4 class="mb-0" id="totalProfesionesCounter">{{ $profesiones->total() }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex-grow-1">
                                <div class="search-box position-relative">
                                    <input type="text" id="searchInput" class="form-control search form-control-lg ps-5"
                                        placeholder="Buscar profesión..." value="{{ request('search') ?? '' }}">
                                    <i
            ...
            <div class="col-12">
                <div class="card border border-light shadow-sm">
                    <div class="card-header border-bottom-dashed d-flex align-items-center justify-content-between">
                        <h5 class="card-title mb-0 fw-bold">Listado de Profesiones</h5>
                        <div class="dropdown">
                            ...
                            <table class="table table-borderless table-centered align-middle table-nowrap mb-0">
                                <thead class="table-light text-muted fw-semibold">
                                    <tr>
                                        ...
                                        <th class="px-3 py-3">Nombre de la Profesión</th>
                                        <th class="px-3 py-3 text-center" width="15%">Estado</th>
                                        <th class="px-3 py-3 text-end" width="15%">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody id="profesionesTableBody">
                                    @include('admin.profesiones.partials.table-body')
                                </tbody>
                            </table>
                        </div>
                        @if ($profesiones->total() > 0)
                            <div class="d-flex justify-content-between align-items-center mt-4 flex-wrap gap-2">
                                <div class="results-count text-muted">
                                    Mostrando <span class="fw-medium">{{ $profesiones->firstItem() }}</span> a
                                    <span class="fw-medium">{{ $profesiones->lastItem() }}</span> de
                                    <span class="fw-medium">{{ $profesiones->total() }}</span> resultados
                                </div>
                                <div class="pagination-container">
                                    {{ $profesiones->appends(request()->input())->links('pagination::bootstrap-5') }}
                                </div>
                            </div>
                        @endif
                    </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-primary-subtle p-3 position-relative">
                    <h5 class="modal-title fw-bold text-primary" id="registrarLabel">
                        <i class="ri-add-line me-2 align-bottom"></i>Registrar Nueva Profesión
                    </h5>
                ...
                <form id="addForm" class="needs-validation" novalidate>
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-lg-12">
                                <div class="mb-3">
                                    <label for="nombre_registro" class="form-label fw-medium">Nombre de la Profesión <span
                                            class="text-danger">*</span></label>
                                    <input type="text" id="nombre_registro" name="nombre"
                                        class="form-control form-control-lg" placeholder="Ej: Ingeniero, Médico, Abogado"
                                        required>
                                    <div class="invalid-feedback">Por favor ingresa el nombre de la profesión</div>
                    ...
                    <div class="modal-footer bg-light p-3">
                        ...
                        <button type="submit" class="btn btn-primary btn-lg addBtn" disabled>
                            ...
                            <span class="submit-text">Registrar Profesión</span>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-warning-subtle p-3 position-relative">
                    <h5 class="modal-title fw-bold text-warning" id="modalModificarLabel">
                        <i class="ri-edit-line me-2 align-bottom"></i>Editar Profesión
                    </h5>
                ...
                <form id="updateForm" class="needs-validation" novalidate>
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id" id="profesioneId">
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-lg-12">
                                <div class="mb-3">
                                    <label for="nombre_edicion" class="form-label fw-medium">Nombre de la Profesión <span
                                            class="text-danger">*</span></label>
                    ...
                    <div class="modal-footer bg-light p-3">
                        ...
                        <button type="submit" class="btn btn-warning btn-lg updateBtn" disabled>
                            ...
                            <span class="submit-text">Actualizar Profesión</span>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <form id="deleteForm">
                    ...
                    <div class="modal-body p-4 text-center">
                        ...
                        <h4 class="mb-3 fw-bold">¿Estás seguro de eliminar esta profesión?</h4>
                        <p class="text-muted mb-0">Esta acción no se puede deshacer. Si hay registros asociados a esta
                            profesión, deberás actualizarlos manualmente.</p>
    ...
    <script>
        $(document).ready(function() {
                        ...
                        success: function(res) {
                            if (res.exists) {
                                feedback.addClass('text-danger').html(
                                    '<i class="ri-error-warning-line me-1"></i> Esta profesión ya existe'
                                );
                                submitBtn.prop('disabled', true);
                                input.addClass('is-invalid').removeClass('is-valid');
                            } else {
            ...
            $('#nombre_edicion').on('input', function() {
                ...
                debounceTimer = setTimeout(() => {
                    $.ajax({
                        url: "{{ route('admin.profesiones.verificaredicion') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            nombre: nombre,
            ...
            $('#addForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.profesiones.registrar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            showToast('error', res.msg || 'Error al registrar la profesión');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al registrar la profesión. Intenta nuevamente.';
            ...
            $('#updateForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.profesiones.modificar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            showToast('error', res.msg || 'Error al actualizar la profesión');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al actualizar la profesión. Intenta nuevamente.';
                        if (xhr.responseJSON?.errors?.nombre) {
                            errorMsg = xhr.responseJSON.errors.nombre[0];
                        } else if (xhr.responseJSON?.msg) {
                            errorMsg = xhr.responseJSON.msg;
            ...
            $('#deleteForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.profesiones.eliminar') }}",
                    type: "POST",
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
                            ...
                            showToast('error', res.msg || 'Error al eliminar la profesión');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'Error al eliminar la profesión. Intenta nuevamente.';
            ...
            function loadResults(search = '') {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.html) {
                            $('#profesionesTableBody').html(response.html);
                            ...
                            if (response.total !== undefined) {
                                ...
                                // Actualizar contador
                                $('#totalProfesionesCounter').text(response.total);
                            }
