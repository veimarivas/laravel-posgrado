...
class DetalleParticipantesExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths, WithTitle, WithEvents, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        return collect($this->participantes)->map(function ($participante) {
            return [
                ...
                'DEPARTAMENTO' => $participante['departamento'] ?? 'No registrado',
                'PROFESIÓN' => $participante['profesion'] ?? 'No registrado',
                'GRADO ACADÉMICO' => $participante['grado_academico'] ?? 'No registrado',
                'INSTITUCIÓN' => $participante['institucion'] ?? 'No especificada',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("B{$dataStartRow}:C{$dataEndRow}")->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER);

                $sheet->getStyle("I{$dataStartRow}:J{$dataEndRow}")->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER);

                $sheet->getStyle("P{$dataStartRow}:P{$dataEndRow}")->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER);

                $sheet->getStyle("Q{$dataStartRow}:Q{$dataEndRow}")->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER);
                ...
                $sheet->getStyle("K{$dataStartRow}:L{$dataEndRow}")->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
                ...
                $sheet->getStyle("M{$dataStartRow}:O{$dataEndRow}")->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
                ...
                $sheet->getStyle("P{$dataStartRow}:Q{$dataEndRow}")->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
                ...
                $sheet->getStyle("I{$dataStartRow}:J{$dataEndRow}")->applyFromArray([
                    'font' => [
                        'color' => ['rgb' => '27AE60']
                    ]
                ]);
                ...
                for ($row = $dataStartRow; $row <= $dataEndRow; $row++) {
                    $profesion = $sheet->getCell("{$profesionCol}{$row}")->getValue();
                    ...
                    if (
                        ...
                        strpos(strtolower($profesion), 'licenciado') !== false ||
                        ...
                        $bgColor = 'F4ECF7';
                    } elseif (strpos(strtolower($profesion), 'doctor') !== false) {
                ...
                $estudiosCol = 'P';
                for ($row = $dataStartRow; $row <= $dataEndRow; $row++) {
                    $numEstudios = $sheet->getCell("{$estudiosCol}{$row}")->getValue();
                ...
                $conCorreo = count(array_filter($this->participantes, function ($p) {
                    return !empty($p['correo']) && $p['correo'] !== 'Sin correo';
                }));
                $conCelular = count(array_filter($this->participantes, function ($p) {
                    return !empty($p['celular']) && $p['celular'] !== 'Sin celular';
                }));
                ...
                $sheet->setCellValue("A{$statsRow1}", "Total de participantes:");
                ...
                $sheet->setCellValue("F{$statsRow1}", "(" . round(($conCorreo / $totalParticipantes) * 100, 1) . "%)");

                $sheet->setCellValue("H{$statsRow1}", "Con celular registrado:");
                $sheet->setCellValue("I{$statsRow1}", $conCelular);
                $sheet->setCellValue("J{$statsRow1}", "(" . round(($conCelular / $totalParticipantes) * 100, 1) . "%)");

                $sheet->setCellValue("A{$statsRow2}", "Departamento principal:");
                $sheet->setCellValue("B{$statsRow2}", $topDepartamento);
                $sheet->setCellValue("C{$statsRow2}", $totalTopDepto . " participantes");
                ...
                $totalEstudios = array_sum(array_map(function ($p) {
                    return count($p['estudios'] ?? []);
                }, $this->participantes));
                $promedioEstudios = $totalParticipantes > 0 ? $totalEstudios / $totalParticipantes : 0;

                $sheet->setCellValue("A{$statsRow3}", "Total estudios registrados:");
                $sheet->setCellValue("B{$statsRow3}", $totalEstudios);

                $sheet->setCellValue("D{$statsRow3}", "Promedio de estudios por participante:");
                $sheet->setCellValue("E{$statsRow3}", round($promedioEstudios, 1));
                ...
                $sheet->setCellValue("C{$statsRow4}", $totalTopProf . " participantes");
                ...
                $sheet->getStyle("A{$statsRow1}:Q{$statsRow4}")->applyFromArray($resumenStyle);
